...
def input_to_point_set(path):
    ...
    for section in path:
        ...
        new_points, current_pos = trace_section(direction, distance, current_pos, step_num)
        step_num += distance
        points = new_points | points
    return points
    
def trace_section(direction, distance, current_pos, step_num):
    ...
    if direction in ('L', 'R'):
        if direction == 'L':
            ...
            sign = 1
        new_points = {(x_pos + x * sign, y_pos): step_num + x for x in range(1, distance + 1)}
    ...
    if direction in ('D', 'U'):
        if direction == 'D':
            ...
            sign = 1
        new_points = {(x_pos, y_pos + y * sign): step_num + y for y in range(1, distance + 1)}
...
def get_lowest_distance(data):
    ...
    distances = {points1[position] + points2[position] for position in intersections}
...
def tests():
    ...
    assert get_lowest_distance(test_input1) == 610
    assert get_lowest_distance(test_input2) == 410
